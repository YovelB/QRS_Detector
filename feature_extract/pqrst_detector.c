...
static uint16_t detect_r_peak(volatile const float* buffer, uint16_t size) {
  float max_val = 0.0f;
  uint16_t idx = 0;
...
static uint16_t detect_q_wave(volatile const float* buffer) {
  ...
  /* search backwards from r peak within qrs window */
  int i = g_r_idx;
  for(; i >= MAX(0, g_r_idx - QRS_WINDOW_MAX); i--) {
    if(buffer[i] < -Q_WAVE_THRESHOLD) {
      idx = i;
      break;
    }
  }
...
static uint16_t detect_s_wave(volatile const float* buffer, uint16_t size) {
  uint16_t idx = g_r_idx;

  /* search forwards from r peak within qrs window */
  uint16_t i = g_r_idx;
  for(; i < MIN(size, g_r_idx + QRS_WINDOW_MAX); i++) {
    if(buffer[i] < -S_WAVE_THRESHOLD) {
      idx = i;
      break;
    }
  }
...
static uint16_t detect_t_wave(volatile const float* buffer, uint16_t size) {
  float max_val = -1000.0f;
  uint16_t idx = 0;

  /* search for local maximum after s wave within qt window */
  uint16_t i = g_s_idx;
  for(; i < MIN(size, g_s_idx + QT_WINDOW_MAX); i++) {
...
wave_points_t ecg_detect_pqrst(volatile const float* buffer, uint16_t size) {
  ...
  g_r_idx = detect_r_peak(buffer, size);
  ...
  g_s_idx = detect_s_wave(buffer, size);
  ...
  points.t_idx = detect_t_wave(buffer, size);
...
{
    wave_intervals_t intervals = {0};
    float samples_to_ms = 1000.0f / SAMPLE_FREQ;  /* conversion for 80Hz sampling rate */

    /* calculate pr interval (p start to q start) */
    intervals.pr_interval = (float)(points->q_idx - points->p_idx) * samples_to_ms;

    /* calculate qrs duration (q start to s end) */
    intervals.qrs_duration = (float)(points->s_idx - points->q_idx) * samples_to_ms;

    /* calculate qt interval (q start to t end) */
    intervals.qt_interval = (float)(points->t_idx - points->q_idx) * samples_to_ms;
